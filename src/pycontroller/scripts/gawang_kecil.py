...
def run(time, dets, track_ball, head_control):
    ...
    if state == BALL_SEARCHING_INIT:
        ...
        # yaw = yaw_init
        walk.setTarget()
        set_state(WALK_INIT_STARTING, 1)
        ...
        if timedEnd:
            setwalkcmd("stop")
            set_state(AUTO_READY_TURNING, 5)
        ...
        if timedEnd:
            setwalkcmd("start")
            walk.setTarget(0, -0.7, 0.02)
            set_state(SHOOTING_2_WALK_POST, 5)
    
    elif state == SHOOTING_POST:
        if timedEnd:
            enableWalk()
            set_state(SHOOTING_2_WALK, 1)
        ...
        setwalkcmd("start")
        # yaw = yaw_init
        walk.setTarget(0, 0.9, 0.06)
        set_state(AUTO_READY_INIT_STARTING, 30) #30
        ...
        if timedEnd:
            ...
            set_state(WALK_STOP_INIT, 11.5) #11.5

    elif state == AUTO_READY_TURNING:
        if timedEnd:
            if bt.isEnabled and not gt.enabled and infer.ball_lock: 
                if head_pitch <= 1 and head_yaw <= - 0.7 :
                    setwalkcmd("start")
                    set_state(WALK_INIT, 3)
                    print("head py:"+str(head_pitch)+", "+str(head_yaw))
                elif head_pitch <= 1 and head_yaw >= 0.7 :
                    setwalkcmd("start")
                    set_state(WALK_INIT, 3)
                    print("head py:"+str(head_pitch)+", "+str(head_yaw))
                elif head_pitch < 1 :
