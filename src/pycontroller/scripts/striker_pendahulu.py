...
def run(time, dets, track_ball, head_control):
    ...
    if state == BALL_SEARCHING_INIT:
        ...
        if bt.isEnabled and not gt.enabled and infer.ball_lock: 
            if head_pitch <= -0.46 : move = 0.1   
            if head_pitch <= -0.46 and abs(head_yaw) < 0.16 : #0.1 
                if abs(yaw/2) < 0.25: # 0.25
        ...
        if abs(head_yaw) < 0.1:
            yaw_control = plus_or_min(head_yaw, 0.19) #
        
        if head_yaw >= -0.19 and head_yaw <= 0.:
