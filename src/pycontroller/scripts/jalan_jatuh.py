...
def run(time, dets, track_ball, head_control):
    ...
    if state == BALL_SEARCHING_INIT:
        svhd_pitch = head_pitch
        svhd_yaw = head_yaw
        print("head pitch:"+str(head_pitch))
        print("head yaw:"+str(head_yaw))
        if head_pitch + 0.02 >= svhd_pitch >= head_pitch - 0.02 and head_yaw + 0.02 >= svhd_yaw >= head_yaw - 0.02:
            print("head pitch:"+str(head_pitch))
            print("head yaw:"+str(head_yaw))
            print("frame:"+str(ball_stay))
            ball_stay += 1
        else :
            print("head pitch:"+str(head_pitch))
            print("head yaw:"+str(head_yaw))
        ...
        if ball_stay >= 30 :
            ball_stay = 0
            set_state(BALL_SEARCHING_TURNING)
            


    elif state == BALL_SEARCHING:   
        # move = 2 #0.6
        if infer.ball_lock and bt.isEnabled and not gt.enabled:
                    if head_pitch < 1 :
                        wlfirst = False
                        print("head py:"+str(head_pitch)+", "+str(head_yaw))
                        set_state(WALK_2_SHOOT_INIT_1)  
                    elif head_pitch <= 1 and head_yaw <= - 0.5 :
                        print(right)
                        right = True
                        wlfirst = True
                        print("head py:"+str(head_pitch)+", "+str(head_yaw))
                        set_state(AUTO_READY_TURNING) #kanan
                    elif head_pitch <= 1 and head_yaw >= 0.5 :
                        right = False
                        print(right)
                        wlfirst = True
                        print("head py:"+str(head_pitch)+", "+str(head_yaw))
                        set_state(AUTO_READY_TURNING) #kiri
        ...
        #     set_state(BALL_SEARCHING_TURNING, 1)
        #     ball_search_loss = 0
        
    elif state == BALL_SEARCHING_TURNING:
        if timedEnd:
            walk.setTarget(-0.4, -0.7, 0.364) #0.3 #0.8
            set_state(WALK_STOP_INIT, 5.4879)

    elif state == GOAL_SCAN_INIT: # init scan
        walk.setTarget(0.5, 0.3, 0.3)
        ...
        if timedEnd:
            ...
            enableAction()
            walk.setTarget()
            print("wl :" +str(wlfirst))
            set_state(WALK_2_SHOOT_INIT_2, 0.5)
    
    elif state == WALK_2_SHOOT_INIT_2:
        # if timedEnd:
            enableAction()
            set_state(WALK_2_SHOOT_INIT_3, 0.5)

    elif state == WALK_INIT:
        setwalkcmd("start")
        # yaw = yaw_init
        walk.setTarget(0, 0.7, 0.03)
        ...
        if timedEnd :
            walk.setTarget()
            set_state(SHOOTING_2_WALK_POST, 3)
        ...
        if timedEnd:
            ...
            if right :
                    print("wl :" +str(wlfirst))
                    print(right)
                    # walk.setTarget(2.8, -0.1, 0)
                    # walk.setTarget(1.1, -0.1, 0.3)
                    walk.setTarget(1.1, 0.18, 0.435789)
                    #walk.setTarget()
                    set_state(BALL_SEARCHING_TURNING, 8)
            else :
                    print(right)
                    print("wl :" +str(wlfirst))
                    # walk.setTarget(2.8, -0.1, 0)
                    walk.setTarget(-1.1, 0.25, -0.3547)
                    # walk.setTarget(-1.1, -0.18, 0.435789)
                    #walk.setTarget()
                    set_state(BALL_SEARCHING_TURNING, 8)
        ...
        if timedEnd :
            ...
            if wlfirst : 
                if right : 
                    ...
                    # fall = True
                    right = True
                    # if not bt.isEnabled and not infer.ball_lock:
                    #     ball_search_loss += 1
                    #     print("frame :" +str(ball_search_loss))
                    # else :
                    #     ball_search_loss = 0
                    #     print("frame :" +str(ball_search_loss))

                    # if ball_search_loss > 30 :
                    set_state(SHOOTING, 3)
                else :
                    print(right)
                    right = False
                    playAction(108)
        ...
        if timedEnd:
            if bt.isEnabled and infer.ball_lock:
                        ball_search_loss = 0
                        print("frame :" +str(ball_search_loss))
            else :
                        ball_search_loss += 1
            ...
            if ball_search_loss > 30 :
                set_state(WALK_2_STANDUP_INIT, 1)
        ...
        if timedEnd:
            ...
            setwalkcmd("stop")
            infer.ball_lock = False
            head_control[0] = 1.0
            head_control[1] = 0.0
            infer.lost_count = infer.max_lost
            bt.isEnabled = True
            set_state(BALL_SEARCHING)
        ...
        if timedEnd:
            ...
            if right : 
                print(right)
                # walk.setTarget(-0.5, -0.5, 1.1)
                walk.setTarget(0.05, 0, 1.58)
                # walk.setTarget(0, -0.5, -0.2)
                # if ypr[0] >= -50 :
                right = True
                set_state(AUTO_READY_POSITIONING, 3.2)
            else:
                print(right)
                # walk.setTarget(-0.5, -0.5, -0.8) #kiri yaa
                walk.setTarget(0.5, 0, -0.8) #kiri yaa
                # walk.setTarget(1.5, -0.9, -0.1)
                # if ypr[0] <= 200 :
                right = False
                set_state(AUTO_READY_POSITIONING, 4.3)
        ...
        if timedEnd:
            ...
            # if right :
            #     walk.setTarget(1, -0.7, 0.1)
            #     set_state(WALK_STOP_INIT, 10)
            # else :
            #     walk.setTarget(1, -0.7, 0.1)
            #     set_state(WALK_STOP_INIT, 10)
            #walk.setTarget(-1, -0.1, 0.1)
            print("wl :" +str(wlfirst))
            if wlfirst :
                set_state(SHOOTING_POST)
            else :
                right = True
                set_state(WALK_STOP_INIT)
        ...
        if timedEnd:
            ...
            if not right :
                    ...
                    # walk.setTarget(2.8, -0.1, 0)
                    # walk.setTarget(1.1, -0.1, 0.3)
                    walk.setTarget(1.1, 0.18, 0.3) #jalan ke kiri
                    #walk.setTarget()
                    right = False
                    set_state(WALK_2_SHOOT_INIT_1, 10)
            else :
                    print(right)
                    # walk.setTarget(2.8, -0.1, 0)
                    walk.setTarget(-1.5, 0.18, -0.15) #jalan ke kanan
                    # walk.setTarget(-1.1, -0.18, 0.435789)
                    #walk.setTarget()
                    right = True
                    set_state(WALK_2_SHOOT_INIT_1, 10)
        ...
        if timedEnd:
            setwalkcmd("stop")
            set_state(WALK_2_STANDUP_INIT_2, 0.5)

    elif state == WALK_2_STANDUP_INIT_2:
        if timedEnd:
            enableAction()
            set_state(ACTION_2_STANDUP_INIT, 0.5)
