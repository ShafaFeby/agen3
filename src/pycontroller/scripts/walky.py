...
import striker_gabung as striker
import strikery as fall
import pinalti as pinalti
...
def button_callback(data):
    if data.data == "start":
        ...
        if striker.state == striker.PAUSE:
            striker.set_state(striker.AUTO_READY_TURNING)
        ...
        print("mode pressed")
        if pinalti.state == pinalti.PAUSE:
            pinalti.set_state(pinalti.BALL_SEARCHING)
        else: walk_toggle(False)
    elif data.data == "mode_long":
        print("mode_long pressed")
        if fall.state == fall.PAUSE:
            fall.set_state(fall.BALL_SEARCHING)
        else: walk_toggle(False)
    else:
...
def main():
    ...
    fall.init(goaltracker, inference, ball_tracking, walking, setWalkParams, setWalkCmd)
    fall.init_action(pubMotionIndex, isActionRunning, pubEnaMod, pubEnableOffset)

    pinalti.init(goaltracker, inference, ball_tracking, walking, setWalkParams, setWalkCmd)
    pinalti.init_action(pubMotionIndex, isActionRunning, pubEnaMod, pubEnableOffset)
    ...
    while not rospy.is_shutdown():
        # try:
            ...
            fall.run(toc, dets, track_ball, head_control)

            pinalti.run(toc, dets, track_ball, head_control)
