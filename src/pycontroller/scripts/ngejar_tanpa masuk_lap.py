...
z_amp_turning = 0.032 #0.03
z_amp_normal = 0.026

yaw_compe_ball_align = 0.1
time_multi_goal_align_yaw = 4.5
time_multi_goal_align = 6.5 #5
yaw_x_turning_max = 1.9 #1.5
max_yaw_gain = 0.35 #0.42
...
def run(time, dets, track_ball, head_control):
    ...
    if state == BALL_SEARCHING_INIT:
        ...
        if bt.isEnabled and not gt.enabled and infer.ball_lock: 
            if head_pitch < -0.5: move = 0.3   
            if head_pitch < -0.5 and abs(head_yaw) < 0.1 :
                # if abs(yaw/2) < 0.25:
        ...
        yaw_gain = clamp(turn_gain, -0.40, 0.40)
        ...
        if timedEnd:
            ...
            if not gt.enabled:
                if goal.found:
                    ...
                    arahgawang = turn_gain

                    yaw_gain = clamp(turn_gain, -max_yaw_gain, max_yaw_gain)
                    walk.setTarget(-turn_gain , 0.045, yaw_gain)
                    setwalkparams(["z_move_amplitude", z_amp_turning])
    ...
    elif state == SWING_2_PARAM:
            setwalkcmd("start")
            # yaw = yaw_init
            setwalkparams(["z_move_amplitude", 0.01])
            walk.setTarget()
            set_state(SWING_2_PARAM_2, 0.4)

    elif state == SWING_2_PARAM_2:
        if timedEnd :
            setwalkcmd("start")
            setwalkparams(["z_move_amplitude", 0.013])
            walk.setTarget()
            set_state(SWING_2_PARAM_3, 0.4)

    elif state == SWING_2_PARAM_3:
        if timedEnd :
            setwalkcmd("start")
            setwalkparams(["z_move_amplitude", 0.016])
            walk.setTarget()
            set_state(GOAL_SCAN_INIT, 0.4)
