    ...
    if state == BALL_SEARCHING_INIT:
        ...
        if timedEnd :   
            ...
            if bt.isEnabled and not gt.enabled and infer.ball_lock:  
                if head_pitch <= 1 and head_yaw <= - 0.5 :
                    ball_set_target += 1
                ...
                if ball_set_target > 10 :
                    set_state(WALK_2_SHOOT_INIT_1, 3)
            else:
                ball_search_loss += 1
        ...
        if timedEnd:
            walk.setTarget()
            set_state(WALK_STOPING, 3.5)

    elif state == WALK_STOPING:
        if timedEnd:
            setwalkcmd("stop")
            
            set_state(RESTART_KIPER, 10)
            
    
    elif state == RESTART_KIPER:
            set_state(BALL_SEARCHING, 20)
        ...
        if timedEnd:
            if right : 
                ...
                walk.setTarget(0.05, 0, 1.2) #1.58
                # walk.setTarget(0, -0.5, -0.2)
                # if ypr[0] >= -50 :
                set_state(AUTO_READY_POSITIONING, 4.18) #3.2
            else:
