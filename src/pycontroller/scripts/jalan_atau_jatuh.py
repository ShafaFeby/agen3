...
POSITIONING_INIT_ST = 35
POSITIONING_STOP_ST = 36
...
SWING_PARAM_BACK_1 = 42

SWING_PARAM_BACK_2 = 43

SWING_PARAM_BACK_3 = 44
...
states_dict = {
    ...
    35 : "POSITIONING_INIT_ST",
    36 : "POSITIONING_STOP_ST",
    37 : "SWING_PARAM_1", 
    38 : "SWING_PARAM_2", 
    39 : "SWING_PARAM_3", 
    40 : "SWING_PARAM_4", 
    41 : "SWING_PARAM_5", 
    42 : "SWING_PARAM_BACK_1",
    43 : "SWING_PARAM_BACK_2",
    44 : "SWING_PARAM_BACK_3",
}
...
def run(time, dets, track_ball, head_control):
    ...
    # print("head pitch:"+str(head_pitch))
    # print("head yaw:"+str(head_yaw))

    # print("head py:"+str(head_pitch)+", "+str(abs(head_yaw))+","+str(abs(yaw/2)))
    ...
    if state == BALL_SEARCHING_INIT:
        if timedEnd:
            setwalkcmd("start")
            # walk.setTarget(0.3, -0.6, 0.8)
            walk.setTarget(-0.4, -0.7, 0.364)
            set_state(POSITIONING_INIT_ST, 10)

    elif state == BALL_SEARCHING:
        move = 0.6
        print("head py:"+str(head_pitch)+", "+str(abs(head_yaw))+","+str(abs(yaw/2)))
        if bt.isEnabled and not gt.enabled and infer.ball_lock: 
            if head_pitch <= -0.62 : move = 0.1
            if head_pitch <= -0.62 :
                if head_yaw > 0.2 and head_yaw < 0.35 :
                    print("head py:"+str(head_pitch)+", "+str(abs(head_yaw))+","+str(abs(yaw/2)))
                    right = False
                    set_state(GOAL_SCAN_INIT)
                elif head_yaw < -0.25 and head_yaw > -0.4 :
                    print("head py:"+str(head_pitch)+", "+str(abs(head_yaw))+","+str(abs(yaw/2)))
                    right = True
                    set_state(GOAL_SCAN_INIT)
            ball_search_loss = 0
        ...
        if head_yaw >= -0.3 and head_yaw <= 0.3:
            walk.setTarget(0.0, move, yaw_control)

        else:
            walk.setTarget(0.0, 0.0, yaw_control)

        if ball_search_loss > 30:
    ...
    elif state == SWING_PARAM_1:
            setwalkcmd("start")
            # yaw = yaw_init
            setwalkparams(["z_move_amplitude", 0.01])
            walk.setTarget()
            set_state(SWING_PARAM_2, 0.4)

    elif state == SWING_PARAM_2:
        if timedEnd :
            setwalkcmd("start")
            setwalkparams(["z_move_amplitude", 0.014])
            walk.setTarget()
            set_state(SWING_PARAM_3, 0.4)

    elif state == SWING_PARAM_3:
        if timedEnd :
            setwalkcmd("start")
            setwalkparams(["z_move_amplitude", 0.018])
            walk.setTarget()
            set_state(SWING_PARAM_4, 0.4)

    elif state == SWING_PARAM_4:
        if timedEnd :
            setwalkcmd("start")
            setwalkparams(["z_move_amplitude", 0.022])
            walk.setTarget()
            set_state(SWING_PARAM_5, 0.4)

    elif state == SWING_PARAM_5:
        if timedEnd :
            setwalkcmd("start")
            setwalkparams(["z_move_amplitude", 0.026])
            walk.setTarget()
            set_state(WALK_INIT, 0.4)
        ...
        if timedEnd :
            ...
            # yaw = yaw_init
            walk.setTarget()
            set_state(WALK_INIT_STARTING, 0.4)
        ...
        if timedEnd:
            ...
            # walk.setTarget(0.3, -0.6, 0.8)
            walk.setTarget(-0.4, -0.7, 0.364)
            set_state(SHOOTING_2_WALK_POST, 10)
        ...
        if timedEnd:
            ...
            setwalkcmd("start")
            walk.setTarget()
            set_state(SHOOTING_2_WALK, 0.5)

    elif state == SWING_PARAM_BACK_1:
        if timedEnd:
            enableWalk()
            setwalkcmd("start")
            walk.setTarget()
            set_state(SHOOTING_POST, 0.4)

    elif state == SHOOTING_INIT:
        if right :
            playAction(139)
            set_state(SHOOTING, 5)
        else :
            right = True
            playAction(144)
            set_state(SHOOTING, 5)
        ...
        if timedEnd:
            stopAction()
            set_state(SWING_PARAM_BACK_1, 0.4)

    elif state == SHOOTING_2_WALK_POST:
        if timedEnd:
            set_state(POSITIONING_INIT_ST, 3)
    
    elif state == AUTO_READY_INIT:
        setwalkcmd("start")
        # yaw = yaw_init
        walk.setTarget(0, 0.7, 0.07)
        set_state(AUTO_READY_INIT_STARTING, 43) #30

    elif state == AUTO_READY_INIT_STARTING:
        if timedEnd:
            enableWalk()
            set_state(AUTO_READY_POSITIONING, 3)
    
    elif state == AUTO_READY_POSITIONING:
        if timedEnd:
            setwalkcmd("start")
            walk.setTarget(0.5, 0, -0.8)
            set_state(POSITIONING_INIT_ST, 11.5) #11.5

    elif state == AUTO_READY_TURNING:
        if timedEnd:
            if bt.isEnabled and not gt.enabled and infer.ball_lock: 
                if bt.isEnabled and not gt.enabled and infer.ball_lock: 
                    if head_pitch < 1 and head_yaw == -0.3 :
                        ball_set_target = 0
                        print("head py:"+str(head_pitch)+", "+str(head_yaw))
                        print("frame :" +str(ball_set_target))
                    elif head_pitch < 1 and head_yaw == 0.3 :
                        ball_set_target = 0
                        print("head py:"+str(head_pitch)+", "+str(head_yaw))
                        print("frame :" +str(ball_set_target))
                    elif head_pitch < 1 and head_yaw <= - 0.3 :
            ...
            if ball_set_target > 35 : #magenta , nek cyan itu 35
                setwalkcmd("start")
                set_state(SWING_PARAM_1)
                print("frame :" +str(ball_set_target))
                ball_set_target = 0
        ...
        set_state(AUTO_READY_INIT, play_delay)
        ...
        if timedEnd:
            ...
            setwalkcmd("start")
            walk.setTarget()
            set_state(AUTO_READY_POST, 3)

    elif state == POSITIONING_INIT_ST:
        if timedEnd:
            setwalkcmd("start")
            walk.setTarget()
            set_state(POSITIONING_STOP_ST, 3)

    elif state == POSITIONING_STOP_ST:
        if timedEnd:
            setwalkcmd("stop")
