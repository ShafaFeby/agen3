...
# pubSend = rospy.Publisher('/send_komunikasi', Int32, queue_size=10)
    ...
    try:
        while True:
            ...
            if cmd == 'torque_on':
                ...
                striker.set_state(striker.ACTIONWALK_MODULE)
            elif cmd == 'set_state':
...
def walk_toggle(_on):
    ...
    if _on:
        striker.set_state(striker.BERHENTI_JALAN,  2.0)
    else: striker.set_state(striker.BERHENTI_JALAN,  1.0)
...
def gamestate_callback(data):
    ...
    if(new_gc_state == "play" and new_gc_state != gc_state):
        striker.set_state(striker.MEMULAI)
        gc_state = new_gc_state
    elif(new_gc_state == "ready" and new_gc_state != gc_state):
        striker.set_state(striker.JALAN_DITEMPAT, 3)
        gc_state = new_gc_state

def button_callback(data):
    if data.data == "start":
        print("start pressed")
        striker.set_state(striker.SELESAI_MOTION)
        ...
        print("mode pressed")
        striker.set_state(striker.AKTIFKAN_MOTION)
        # striker.zero_ypr()
...
def listener_komunikasi(data) :
      rospy.loginfo(rospy.get_caller_id() + "I heard %s", data.data)
      print("komunikasi : " + str(data.data))
      if str(data.data) == "1" :
          striker.set_state(striker.SIAP2)
      elif str(data.data) == "2" :
          striker.set_state(striker.AKTIFKAN_MOTION)
      elif str(data.data) == "0" :
          striker.set_state(striker.BERHENTI_JALAN) 
...
def main():
    ...
    rospy.Subscriber("/receive_komunikasi", Int32, listener_komunikasi)
    ...
    while not rospy.is_shutdown():
        # try:
            # data = Int32()
            # data.data = 9  # Assign the value to publish
            # # rospy.loginfo(f"Publishing: {data.data}")
            # pubSend.publish(data)  # Publish the data
